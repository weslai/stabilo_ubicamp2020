...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=52):
        ...
        self.conv1 = nn.Conv1d(in_channels=13, out_channels=64, kernel_size=7, stride=2, padding=1, bias=True)
        ...
        self.layer3 = self._make_layer(block, 128, layers[1], kernel_size=5, stride=1)
        self.layer4 = self._make_layer(block, 256, layers[2], kernel_size=3, stride=1)
        self.layer5 = self._make_layer(block, 256, layers[2], kernel_size=3, stride=1)
        self.layer6 = self._make_layer(block, 512, layers[3], kernel_size=3, stride=1)
        self.layer7 = self._make_layer(block, 512, layers[3], kernel_size=3, stride=1)
